...
async def gift(client, message):
    ...
    if not message.reply_to_message:
        await message.reply_text("{good_id}You need to reply to a user's message to gift a character!")
        return
    ...
    good = await user_collection.find_one({'id': good_id})

    character = next((character for character in good['characters'] if character['id'] == character_id), None)
    ...
    pending_gifts[(good_id, receiver_id)] = {
        'character': character,
